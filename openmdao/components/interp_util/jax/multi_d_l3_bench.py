...
def diagonal_jacobian(x_data, y_data, x_query):
    ...
    return vmap(lambda i: jacfwd(slinear_interpolate, argnums=2)(x_data, y_data, x_query)[i, i, i])(jnp.arange(x_query.size))
...
print('Openmdao lagrange3:   ', time() - t0)

t0 = time()
for j in range(count):
    f_om, _ = interp_om_fastest.interpolate(x_np, compute_derivative=True)

print('Openmdao lagrange3 3d:', time() - t0)
